...
public class HighlightTextColor  extends CodeArea{
    ...
    public HighlightTextColor setSelectedShapes(ArrayList<Shape> selectedShapes) {
    ...
    public void highlightOnSelect(){
        ...
        if(!selectedShapes.isEmpty()) // If user select a shape.
        {
            for(int z = 0 ; z < selectedShapes.size(); z ++) // Loop for every shape selected.
            {
                ...
                controller.shape.Shape mySelectedShape = ActiveCanvas.getActiveCanvas().getShapeById(id);   // Take the shape with the id.
                ...
                for(int y = 0; y < positions.size(); y++){
                    setStyleClass(positions.get(y), positions.get(y)+mySelectedShape.print().length(), "highlight"); // Highlight the position of the shape.
                }
            }
        }
    }
